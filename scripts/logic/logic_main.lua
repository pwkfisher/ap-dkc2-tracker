...
Donkey Kong Country 2_location = {}
Donkey Kong Country 2_location.__index = Donkey Kong Country 2_location
...
function Donkey Kong Country 2_location.new(name)
    local self = setmetatable({}, Donkey Kong Country 2_location)
    if name then
...
function Donkey Kong Country 2_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = Donkey Kong Country 2_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function Donkey Kong Country 2_location:connect_two_ways(exit, rule)
...
function Donkey Kong Country 2_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function Donkey Kong Country 2_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function Donkey Kong Country 2_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function Donkey Kong Country 2_location:accessibility()
...
-- 
function Donkey Kong Country 2_location:discover(accessibility, keys)
...
entry_point = Donkey Kong Country 2_location.new("entry_point")
